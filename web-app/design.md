...
### 1. すべての入力はリストから選択する
- ポケモン名・持ち物・特性・性格・テラス・わざ・天候・努力値…**自由入力（手打ち）を排し、必ず候補リストから選ぶ**。
  - 名前/持ち物/特性: `OptSelect`（input+datalist の絞り込みコンボ。打鍵で候補を絞れるが値は候補ベース）
  - わざ: `MoveSelect`（タイプ別 optgroup の select）
  - 性格/テラス/天候/ランク: 純 select / トグルボタン
  - 努力値: モード別プリセット select＋スライダー（数値手入力は種族値未登録時のフォールバックのみ）
  - ダメージ計算の攻守選択: **1つの select に「チーム＞全ポケ＞所持」を optgroup で内包**（カテゴリ切替トグルを廃止）
- 狙い: 表記ゆれ・打ち間違い・未登録名の混入を防ぎ、スマホでも片手で完結。

### 2. データは1ファイルを参照（メンテ一元化・token節約）
- アプリ本体は `app-core.js`（共有）/ `app-speed.js` / `app-damage.js` / `app-main.js` に**物理分割**。
  `boot.js` が fetch→連結→Babel変換→eval し**単一スコープ**で動かす（分割しても従来どおり1スコープ）。
  → 1機能を直すとき該当 `app-*.js` だけ読めばよい＝token節約。共有ヘルパは必ず `app-core.js` に置く。
- ページ別パス: `index.html`(全機能) / `damage.html` / `speed.html`。チーム状態は localStorage `pc_team` を全ページ共有。

### 3. ナビ
- ヘッダー＝ページ移動（PCは横ナビ／モバイルはハンバーガー）。フッター＝モバイルのボトムタブ（下スクロールで自動非表示）。
